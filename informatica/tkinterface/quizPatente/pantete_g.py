...
from tkinter import messagebox, ttk # Aggiunto ttk per la progressbar
import csv
...
class Patente(tk.Frame):
    def __init__(self, master=None):
        ...
        self.master.geometry('700x650') # Aumentata leggermente l'altezza
    ...
    def avvia_simulazione(self):
        self.modalita_esame = False
        self.avvia_quiz(False)

    def avvia_esame(self):
        self.modalita_esame = True
    ...
    def schermata_quiz(self):
        ...
        # Progress Bars (Solo in Simulazione)
        if not self.modalita_esame:
            self.crea_progress_bars()
    ...
    def crea_progress_bars(self):
        corrette = 0
        sbagliate = 0
        
        # Calcolo live dei risultati correnti
        for i in range(len(self.domande)):
            if i in self.risposte_utente and self.risposte_utente[i] != "":
                data = self.risposte_utente[i].upper()
                reale = self.domande[i].get('risposta', "").upper()
                if data == reale:
                    corrette += 1
                else:
                    sbagliate += 1

        bar_frame = tk.Frame(self.container, bg='#393D6B')
        bar_frame.pack(fill="x", padx=40, pady=10)

        # Barra Risposte Esatte (Verde)
        tk.Label(bar_frame, text=f"Corrette: {corrette}", bg='#393D6B', fg='#00FF00', font=("Arial", 9)).pack(side="left")
        pb_green = ttk.Progressbar(bar_frame, maximum=30, value=corrette, length=200)
        pb_green.pack(side="left", padx=10)

        # Barra Errori (Rossa)
        tk.Label(bar_frame, text=f"Errori: {sbagliate}", bg='#393D6B', fg='#FF5555', font=("Arial", 9)).pack(side="left", padx=(20, 0))
        pb_red = ttk.Progressbar(bar_frame, maximum=30, value=sbagliate, length=200)
        pb_red.pack(side="left", padx=10)
...
if __name__ == '__main__':
    root = tk.Tk()
    # Stile per personalizzare le barre (opzionale)
    style = ttk.Style()
